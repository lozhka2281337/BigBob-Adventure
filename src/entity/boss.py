...
BOSS_HP            = 3000
BOSS_SPEED         = 180
BOSS_COLOR         = (180, 0, 255)      # базовый фиолетовый

# ─── Пороги фаз (доля от max_hp) ────────────────────────────────────────────
PHASE2_HP_RATIO    = 0.60
PHASE3_HP_RATIO    = 0.30

# ─── Тайминги атак (секунды) ─────────────────────────────────────────────────
ATTACK_COOLDOWN    = {1: 2.5, 2: 2.0, 3: 1.5}   # между атаками по фазам
LASER_CHARGE_TIME  = 1.2
LASER_FIRE_TIME    = {1: 1.0, 2: 1.0, 3: 1.5}
LASER_DMG_INTERVAL = 0.5     # интервал нанесения урона лазером (сек)
LASER_BEAM_WIDTH   = {1: 12, 2: 12, 3: 24}
MELEE_DURATION     = 0.35
MELEE_HIT_FRAC    = 0.5      # доля melee_duration до нанесения удара
MELEE_REACH        = 90
MELEE_ARC          = 160     # градусов
MELEE_DAMAGE       = 2
DASH_DURATION      = 0.25
DASH_COOLDOWN      = {1: 5.0, 2: 4.0, 3: 3.0}
DASH_SPEED         = {1: 500, 2: 550, 3: 700}
SUMMON_DURATION    = 3.5     # время неуязвимости при призыве

# ─── Дистанция (пиксели) ─────────────────────────────────────────────────────
PREFERRED_MIN_DIST = 180
PREFERRED_MAX_DIST = 350
GRENADE_RANGE_MAX  = 400

# ─── Параметры гранаты босса ─────────────────────────────────────────────────
GRENADE_SPEED       = 350
GRENADE_BLAST_R     = 90
GRENADE_FUSE        = 1200   # мс
...
class Boss(Enemy):
    """Босс «Root-Kit» — трёхфазный противник с FSM.

    Фаза 1 (100–60% HP): кайтинг + атаки оружием (граната, меле, лазер).
    Фаза 2 (60–30% HP): призыв миньонов + кратковременная неуязвимость.
    Фаза 3 (<30% HP): агрессивный режим — частые атаки, усиленный лазер, рывки.
    """

    def __init__(self, x: int, y: int, room: pygame.Rect):
        super().__init__(x, y, BOSS_HP, BOSS_SPEED, BOSS_COLOR, room)
        # Переопределяем размер (Boss больше обычного врага)
        self.rect = pygame.Rect(x, y, BOSS_SIZE, BOSS_SIZE)
        self.pos  = pygame.math.Vector2(x, y)

        self.max_hp  = BOSS_HP
        self.damage  = 2          # контактный урон
        self.state   = EnemyState.CHASE  # для совместимости с базовым get_damage
        ...
        # ── Атакующие таймеры ───────────────────────────────────────────────
        self.attack_cooldown  = 2.0   # обратный отсчёт до следующей атаки
        self.attack_timer     = 0.0   # длительность текущей атаки
        ...
        self._laser_end_cache   = None   # вычисляется при рисовании
        ...
        # ── Ссылки, сохраняемые во время update для draw ─────────────────────
    ...
    def draw(self, surface: pygame.Surface, cam_x: float, cam_y: float) -> None:
        ...
        # Метка
        label = self._font_phase.render(
            f"ROOT-KIT  [P{phase_n}]", True, color)
        ...
        elif self.boss_state in (BossState.ATTACK_LASER_CHARGE,
                                  BossState.ATTACK_LASER_FIRE):
    ...
    def _tick_chase_kite(self, world, player, dt: float) -> None:
        ...
        # Обратный отсчёт до атаки
        self.attack_cooldown -= dt
        if self.attack_cooldown <= 0:
            self._pick_attack(dist, phase_n, world, player)
            return

        # Рывок (dash) по отдельному таймеру
    ...
    def _calc_kite_direction(self, player, dist: float) -> pygame.math.Vector2:
        ...
        # Стрейф (перпендикулярное движение)
        strafe = pygame.math.Vector2(-to_player_n.y, to_player_n.x)
        # Меняем направление стрейфа раз в ~3 секунды
    ...
    def _pick_attack(self, dist: float, phase_n: int, world, player) -> None:
        ...
        if dist < 130:
            ...
            # Средняя дистанция: граната или меле
            if random.random() < 0.5:
                self._enter_melee(player)
            else:
                self._enter_grenade(world, player)
        elif has_los:
            # Дальняя дистанция: лазер или граната
            if random.random() < 0.6:
                self._enter_laser_charge(player)
            else:
                self._enter_grenade(world, player)
        else:
            # Нет прямой видимости — граната за угол
            self._enter_grenade(world, player)

    # ── Атака: граната ────────────────────────────────────────────────────────

    def _enter_grenade(self, world, player) -> None:
        # Бросаем гранату немедленно
        grenade = Grenade(
        ...
        self.attack_timer  = 0.5  # пауза после броска

    def _tick_attack_grenade(self, world, player, dt: float) -> None:
        direction = self._calc_kite_direction(player,
                                              self.pos.distance_to(player.pos))
        self.move(world.walls, dt, direction)
    ...
    def _tick_attack_melee(self, world, player, dt: float) -> None:
        # Слегка двигаемся к игроку при замахе
    ...
    def _tick_laser_charge(self, world, player, dt: float) -> None:
        # Во время зарядки почти стоим на месте
        self.attack_timer -= dt
        if self.attack_timer <= 0:
    ...
    def _check_laser_player_damage(self, player) -> None:
        ...
        # clipline принимает два кортежа
    ...
    def _get_laser_end(self, start: pygame.math.Vector2,
                       walls: list[pygame.Rect],
                       max_range: float = 1500.0) -> pygame.math.Vector2:
    ...
    def _start_dash(self, player) -> None:
        ...
        for _ in range(3):
            from projectile.effects import SparkEffect
            self._effects_queue = getattr(self, '_effects_queue', [])
            # (мы не имеем доступа к world здесь, эффекты добавим в tick)

    def _tick_dash(self, world, player, dt: float) -> None:
        phase_n = self.phase.value
        # Добавляем следовые искры
        if random.random() < 0.4:
            world.effects.append(
                SparkEffect(self.rect.centerx, self.rect.centery,
                            PHASE_COLOR[phase_n]))

        self.move(world.walls, dt, self.dash_velocity.normalize())
        # Применяем ускоренное движение напрямую через knockback
        self.knockback = pygame.math.Vector2(
            self.dash_velocity.x, self.dash_velocity.y)
    ...
    def _spawn_minions(self, world) -> None:
        ...
        # Комнаты кроме комнаты босса
        spawn_rooms = [r for r in world.rooms
                       if r != self.room and r.width > 120 and r.height > 120]
        if not spawn_rooms:
            spawn_rooms = [self.room]
        ...
        for EnemyCls, count in groups:
            for _ in range(count):
                ...
                margin = 50
                x = random.randint(room.left + margin,
                                   max(room.left + margin + 1,
                                       room.right - margin - 32))
                y = random.randint(room.top + margin,
                                   max(room.top + margin + 1,
                                       room.bottom - margin - 32))
    ...
    def _draw_melee_arc(self, surface: pygame.Surface,
                        offset_rect: pygame.Rect) -> None:
    ...
    def _draw_laser_visual(self, surface: pygame.Surface,
                           offset_rect: pygame.Rect, phase_n: int) -> None:
        ...
        if self.boss_state == BossState.ATTACK_LASER_CHARGE:
            # Пульсирующий кружок — предупреждение
            pulse  = abs(math.sin(pygame.time.get_ticks() * 0.02)) * 6
            radius = int(10 + pulse)
            pygame.draw.circle(surface, col, start_s, radius)
            pygame.draw.circle(surface, (255, 255, 255), start_s,
                               max(1, radius - 4))
            # Тонкая линия-прицел
            aim_end_w = start_w + self.laser_dir * 200
            aim_end_s = (int(aim_end_w.x - self._cam_x),
                         int(aim_end_w.y - self._cam_y))
            ...
            end_s  = (int(end_w.x - self._cam_x),
                      int(end_w.y - self._cam_y))

            pygame.draw.line(surface, col, start_s, end_s, beam_w)
            pygame.draw.line(surface, (255, 255, 255), start_s, end_s,
                             max(1, beam_w // 3))

            # Искра в точке попадания
            spark_r = int(beam_w * 1.5 + abs(math.sin(
                pygame.time.get_ticks() * 0.05)) * 3)
            pygame.draw.circle(surface, col, end_s, spark_r)
            pygame.draw.circle(surface, (255, 255, 255), end_s,
                               max(2, spark_r // 2))
